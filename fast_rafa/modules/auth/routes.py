...
#             return templates.TemplateResponse(
#                 'login.html',
#                 {'request': request, 'message': message, 'current_user': None},
#             )

#         # Cria o token de acesso
#         access_token = create_access_token(data_payload={'sub': user.username})

#         # Configura a resposta como um redirecionamento
#         response = RedirectResponse(
#             url='/home', status_code=status.HTTP_302_FOUND
#         )
#         response.set_cookie(
#             key='access_token',
#             value=access_token,
#             httponly=True,
#             secure=True,  # Use True apenas se estiver em HTTPS
#         )
...
async def register_page(request: Request, db: Session = Depends(get_session)):
    ...
    # Adicionando 'data_maxima' como um atributo temporário no objeto `user`
    print(data_maxima)
